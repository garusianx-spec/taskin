...
check(await qa.count() === 1, 'custom column appended');
...
await page.getByLabel('نام ستون').fill('تست کیفیت');
await page.getByRole('button', { name: 'افزودن ستون', exact: true }).click();
check(await page.getByText('ستونی با این نام وجود دارد.').isVisible(), 'duplicate column name rejected');
...
check(await qa.getByRole('group', { name: /تهیه سند معماری/ }).count() === 1, 'card dragged into custom column');
...
check(await done.getByRole('group', { name: /تهیه سند معماری/ }).count() === 1, 'checked card moved to Done column');
...
await page.waitForTimeout(200);
check(await qa.getByRole('group', { name: /تهیه سند معماری/ }).count() === 1, 'unchecked card returned to custom column');
...
check(await todo.getByRole('group', { name: /بهینه‌سازی زمان بارگذاری/ }).count() === 1, 'seeded done card unchecked → To Do');
...
check(await prog.getByRole('group', { name: /رفع آسیب‌پذیری/ }).count() === 1, 'keyboard move to next column');
...
check((await confirmDelete.getByRole('combobox', { name: 'ستون مقصد' }).textContent())?.includes('در حال انجام'), 'migrate target defaults to the same-status column');
await confirmDelete.getByRole('button', { name: 'انتقال و حذف ستون' }).click();
check(await page.getByRole('region', { name: 'ستون تست کیفیت' }).count() === 0, 'custom column removed');
check(await prog.getByRole('group', { name: /تهیه سند معماری/ }).count() === 1, 'displaced card now in progress');
...
check(await dialog.isVisible(), 'composer opens from column +');
check((await dialog.getByRole('combobox', { name: 'ستون بورد' }).textContent())?.includes('مسدود'), 'composer preselects custom column');
await dialog.getByLabel('عنوان وظیفه').fill('بررسی وابستگی سرویس پیامک');
await dialog.getByRole('button', { name: 'ایجاد وظیفه' }).click();
check(await page.getByRole('region', { name: 'ستون مسدود' }).getByRole('group', { name: /بررسی وابستگی/ }).count() === 1, 'new task lands in custom column');
